...
typedef struct {
    ...
    void (*background_task_callback)(void);  // 后台任务回调函数
    uint32_t execution_interval;             // 后台任务定时执行时间，单位：毫秒
...
typedef struct {
    ...
    void (*timed_task_callback)(void);  // 定时任务回调函数
    uint32_t execution_interval;        // 定时任务执行时间，单位：毫秒
    uint32_t interval_calc;
...
typedef struct {
    ...
    void (*mian_control_event)(lv_event_t *e);  // 事件处理函数指针 -> 单击进入app页面(内部响应事件码)
